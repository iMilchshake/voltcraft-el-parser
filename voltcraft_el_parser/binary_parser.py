...
def read_binary_file(path):
    ...
    with open(path, 'rb') as f:
        struct_fmt = '=1B'  # read the binary file as unsigned bytes
        struct_len = struct.calcsize(struct_fmt)
        struct_unpack = struct.Struct(struct_fmt).unpack_from
        return list(map(lambda b: b[0], [struct_unpack(chunk) for chunk in iter(partial(f.read, struct_len), b'')]))
...
def parse_chunk(chunk):
    ...
    date = datetime(month=chunk[0], day=chunk[1], year=2000 + chunk[2], hour=chunk[3], minute=chunk[4])

    # get data rows (5-Byte each)
